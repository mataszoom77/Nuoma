...
function addCounter($city){
			...
			$sql =  "SELECT id FROM $lentele WHERE miestas = '$city'";
			if (!$result = $conn->query($sql)) die("Negaliu nuskaityti: " . $conn->error);
				
			if($result->num_rows == 0) {
				$sql = "INSERT INTO $lentele (miestas, kartai) VALUES ('$city', 1)";
				if (!$result = $conn->query($sql)) die("Negaliu įrašyti: " . $conn->error);
			} else {
				$sql =  "UPDATE $lentele SET kartai = kartai + 1 WHERE miestas = '$city'";
				if (!$result = $conn->query($sql)) die("Negaliu nuskaityti: " . $conn->error);
			}
		}
